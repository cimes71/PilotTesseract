...
def test_get_text():
    ...
    for page in temp_list:
        print(page)
def test_get_filtered_df(file_name):
    temp = FileProcessor("/Users/catherineimes/Downloads/mskrecords.pdf", 2, 6)
    temp_df = temp.create_filtered_all_images()
    temp_df.to_csv(file_name, sep='\t', mode='w', index='False')

def test_get_unfiltered_df(file_name):
    temp = FileProcessor("/Users/catherineimes/Downloads/mskrecords.pdf", 2, 6)
    temp_df = temp.create_unfiltered_all_df()
    temp_df.to_csv(file_name, sep='\t', mode='w', index='False')

if __name__ == '__main__':
   #test_get_filtered_df("testout.tsv")
   test_get_unfiltered_df("testall.csv")
